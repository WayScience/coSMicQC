...
![_Extra clustering islands can be seen when looking at morphologyical profiles linked to poor segmentation, which when removed, better reveal patterns in the data. (CFReT)_](./images/CFReT_UMAP_combined.png){width=100%}

![_A representation of ROC AUC scores from multiple random samples taken from a holdout dataset (that has had QC applied) being applied to the QC trained model and no-QC trained model. The QC model outperforms the no-QC model, and the average performance of the QC model is significantly higher than the no-QC model (t-statistic = -72.1, p-value = 0.0). (CFReT)_](./images/bootstrap_plot.png){width=100%}

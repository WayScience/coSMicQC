...
def fixture_jump_cytotable_data(
    ...
    s3_result = cytotable.convert(
        ...
        joins="""
            ...
            FROM
                ...
                LIMIT 8000) AS cytoplasm
            LEFT JOIN (SELECT *
                FROM read_parquet('cells.parquet')
                LIMIT 8000) AS cells ON
    ...
    assert (parquet_file_meta.num_rows, parquet_file_meta.num_columns) == (8000, 5928)
